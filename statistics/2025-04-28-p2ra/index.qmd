...
```{r}
p_prior_df <- tibble(p = 10^seq(-5, 0, length.out=1000), density=prior_p(p), density_log10=density * p)
...
Because we do not have much a prior information about these parameters, we chose the hyperparameters, which set the mean and standard deviation of $\text{logit}(\mu)$, iteratively so that the priors only weakly constrain the posteriors in cases with observed reads. We also included the following considerations:

-   Because we are looking at respiratory viruses, we expect a prior the viral fraction at a given prevalence to be higher in swabs than in wastewater.
...
We extract the posterior samples for further analysis:
...
For small $\mu$, the prevalence must be large to account for detection.
For larger $\mu$, this relationship declines because observing a virus in a swab puts a lower bound on the possible prevalences.
(I'm not sure why you also see this pattern in the unobserved viruses, though.)
...
This makes sense because if we only see a virus a few times and overdispersion is very large, the true prevalence might be lower than we think and our observations are lucky outliers.
