...
Because we do not have much prior information about these parameters, we chose the hyperparameters, which set the mean and standard deviation of $\text{logit}(\mu)$, iteratively so that the priors only weakly constrain the posteriors in cases with observed reads. We also included the following considerations:
