...
model {
  ...
  for (i in 1:N_pools) {
    ...
    for (n_pos in 0:n_swabs[i]) {
      ...
      // Probability of observing r_viral[i] given n_pos positive samples
      real nb_lpmf_val; 
      if (n_pos == 0){
        // If no positive samples in the pool, we should see zero viral reads
        nb_lpmf_val = r_viral[i] == 0 ? 0 : negative_infinity(); 
      } else {
        nb_lpmf_val = neg_binomial_2_lpmf(r_viral[i] | r_total[i] * n_pos * mu_1 / n_swabs[i], n_pos * phi_1);
      }
...
phi_prior <- tibble(
  phi_1 = 10^seq(-2, 1.5, length.out = 1000),
  density = dgamma(phi_1, 2, 1),
  density_log10 = density * phi_1 * log(10),
  ) 
...
posteriors %>%
  ...
  geom_density() +
  geom_line(data = phi_prior, mapping = aes(y = density_log10), color="grey", linetype="dashed") +
  scale_x_log10() +
  facet_wrap(vars(species))
