...
and 2% of no one in Greater Boston being positive (red dotted line).
...
A possible criticism is that this prior puts roughly half of its weight on
prevalences that would be hard to observe in this swab dataset.
If we think of the data as a priori restricted to things that were observed
in at least one pool, this might be inappropriate.
But if we think of the universe of possible viruses as including things that could
be very rare, I think it's reasonable.

An alternative approach, that could be more appropriate for "common viruses" would be:

- People have colds somewhere between 5% and 50% of the time.
- There are somewhere between 10 and 100 cold viruses
- This suggests a prior centered around 5e-3, but with a lot of uncertainty to account for the
  burstiness of outbreaks. Maybe $\text{logit}(p) \sim \text{Normal}(-5, 3)$.
...
Consider a case where you sampled deeply enough that you expect to see roughly 100 reads
from a positive swab in your pool, conditional on getting any at all (as a proxy for a getting a good swab).
Let $\lambda$ be the expected number of reads from a positive swab.
...
```{r}
posteriors %>%
  mutate(p0 = (phi_1 / (mu_swab * 538402.2 + phi_1))^phi_1) %>%
  ggplot(aes(1 - p0, p)) +
  geom_point(alpha = 0.05, size = 0.1) +
  geom_smooth() +
  geom_line(data = tibble(p0 = 1 - 10^seq(-2,0,.1), p = 1 / (547 * (1-p0)))) + 
  scale_x_log10(limits = c(1e-2, 1), labels=scientific) +
  scale_y_log10(limits = c(1e-5, 0.5), labels = scientific) +
  facet_wrap(~species)
```
...
(Caveat that this weak relationship may be an artifact of artificially constraining priors on $\phi$. Maybe if we used a very broad prior we'd find a stronger relationship with $\mu_{ww}$ driven by extreme values.)
...
tibble(rv = c(2, 80, 20), n_pos = c(1, 1, 2)) %>%
  ...
  mutate(
    lam = pmap_dbl(list(phi, rv, n_pos), find_lam),
    p0 = p0(lam, phi),
    p_hat = n_pos / (n_swab * (n_pool - n_pos)) / (1 - p0),
    vlp = var_ln_p(lam, phi, n_pos),
    err = exp(2 * sqrt(vlp))
  ) %>%
  print(n = Inf)
```
...
```
```{r}
mean(swab_metadata$all_reads)
1 / mean(swab_metadata$all_reads)
1 / mean(wastewater_metadata$all_reads)
qlogis(1e-9)
plogis(-10)
```
...
r <- 0.28
mu <- 5000
pnbinom(6000, p = r / mu, size = r, lower.tail = FALSE)
```
